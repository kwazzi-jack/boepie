...
One tool over the generic ``boepie.rag`` engine, pinned to the ``context``
...
``read_context``: hits are bundle file paths the agent opens with its own
...
from boepie.config import DEFAULT_SNIPPET, DEFAULT_TOP_K
from boepie.context import find_bundle, index_root_for
...
async def search_context(input: SearchContextInput) -> str:
    """Locate pages in the curated .boepie/ context bundle for a question.
