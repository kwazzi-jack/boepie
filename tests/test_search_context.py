"""End-to-end tests for the search_context MCP tool.
...
exercises search_context() directly as a plain async function.
...
search_context has no read_context counterpart: its hits are file
...
from boepie.rag.loaders import ContextLoader
from boepie.tools._retrieval import SHORT_SNIPPET_CHARS
from boepie.tools.context import SearchContextInput, search_context
...
async def _build_fixture_index(bundle_dir: Path) -> None:
    ...
    await engine.build(
        ContextLoader(bundle_dir), index_root=index_root_for(bundle_dir), embedding=None
    )
...
@pytest.fixture
async def context_index(tmp_path, monkeypatch):
    """Build a fixture bundle with its own index and run from inside it.

    search_context() discovers the bundle by walking up from the cwd, so
...
async def test_search_context_emits_f2_count_line_and_numbered_blocks(context_index):
    result = await search_context(SearchContextInput(question="wsclean multiscale imaging"))
    first = result.splitlines()[0]
    assert re.match(r'^\d+ hits for "wsclean multiscale imaging" in context$', first)
    hits = _hit_lines(result)
    assert hits
    assert int(first.split()[0]) == len(hits)


async def test_search_context_hit_line_carries_title_and_section(context_index):
    result = await search_context(SearchContextInput(question="wsclean multiscale imaging"))
    hit = _hit_lines(result)[0]
    assert "#" in hit  # section anchor appended to the title


async def test_search_context_hit_line_carries_no_numeric_score(context_index):
...
async def test_search_context_source_line_is_an_openable_bundle_path(context_index):
    result = await search_context(SearchContextInput(question="wsclean multiscale imaging"))
    ...
    for line in sources:
        ...
        assert (context_index.parent / path).exists()
    assert str(context_index) not in result  # no absolute path


async def test_search_context_never_returns_index_md(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(project_dir)
    try:
        result = await search_context(SearchContextInput(question="zzquux routing"))
...
async def test_search_context_emits_no_read_handle(context_index):
    # There is no read_context tool - the source line is the handle.
    result = await search_context(SearchContextInput(question="stimela"))
    assert "read:" not in result


async def test_search_context_emits_no_banned_markdown_or_footer(context_index):
    result = await search_context(SearchContextInput(question="stimela"))
...
async def test_search_context_top_k_respected(context_index):
    result_one = await search_context(SearchContextInput(question="stimela", top_k=1))
    assert len(_hit_lines(result_one)) == 1

    result_many = await search_context(SearchContextInput(question="stimela", top_k=4))
    assert len(_hit_lines(result_many)) == 4


async def test_search_context_top_k_upper_bound_is_twenty(context_index):
    # Aligned with search_literature / search_docs; the old cap was 10.
    assert SearchContextInput(question="x", top_k=20).top_k == 20
    with pytest.raises(ValueError):
        SearchContextInput(question="x", top_k=21)
...
async def test_search_context_snippet_short_is_the_default_and_is_bounded(context_index):
    default = await search_context(SearchContextInput(question="wsclean multiscale imaging"))
    explicit = await search_context(
        SearchContextInput(question="wsclean multiscale imaging", snippet="short")
    )
...
async def test_search_context_snippet_none_drops_bodies_and_shrinks_payload(context_index):
    none_payload = await search_context(
        SearchContextInput(question="wsclean multiscale imaging", snippet="none")
    )
    short_payload = await search_context(
        SearchContextInput(question="wsclean multiscale imaging", snippet="short")
    )
...
async def test_search_context_snippet_full_returns_whole_chunk(context_index):
    full_payload = await search_context(
        SearchContextInput(question="wsclean multiscale imaging", snippet="full")
    )
...
async def test_search_context_without_any_bundle_says_init(tmp_path, monkeypatch):
...
async def test_search_context_with_bundle_but_no_index_says_apply(tmp_path, monkeypatch):
...
async def test_search_context_finds_the_bundle_from_a_nested_subdirectory(
    context_index, monkeypatch
):
    """An agent working in a subdirectory still gets the project's bundle."""
    nested_dir = context_index.parent / "recipes" / "imaging"
...
async def test_bundle_dir_env_override_wins_over_the_walk(tmp_path, context_index, monkeypatch):
    ...
    result = await search_context(SearchContextInput(question="quasar"))
...
async def test_two_projects_do_not_see_each_others_pages(tmp_path, monkeypatch):
    ...
    assert (index_root_for(bundle_a) / "context" / "bm25" / "manifest.json").exists()
    assert (index_root_for(bundle_b) / "context" / "bm25" / "manifest.json").exists()

    try:
        monkeypatch.chdir(project_a)
        await engine.clear_cache()
        result_a = await search_context(SearchContextInput(question="pulsar quasar"))
        assert "alpha" in result_a
        assert "beta" not in result_a

        monkeypatch.chdir(project_b)
        await engine.clear_cache()
        result_b = await search_context(SearchContextInput(question="pulsar quasar"))
